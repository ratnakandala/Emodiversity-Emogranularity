...
def process_single_text(text, text_id): #Adding text_id as an argument to track the row (connectionId)
    ...
    #Change the lists into dicts to keep track of every occurrences' embeddings
    ...
    #The line below converts the input text into a flair Sentence object, which is a data structure used by the flair library to represent and process text. 
    # (in a format that the library understands the input text string)
    ...
    for word in text:
        ...
        if word_type == 'positive':
            ...
            all_pos_word_embeddings.setdefault(word_text, []).append(emb) 
            #If that word isn't a key yet, create a new key with an empty list as its value, 
            #then return that list and append the embedding to it. If the word already has a key in the dict, just return the existing list and
            #append the new embedding to the word's list. This way we keep track of the embeddings for each occurrence of a word separately, while also grouping them by the word text.      

            #Averaging embeddings for repeated positive emotion words in the text
            if word_text in pos_word_avg: #Store unique word averages for reference
            ...
            all_occurrences_neg_words.append(word_text) #Add every occurrence of a negative emotion word to the overall negative words list
            
            #Saving the embeddings for the negative emotion words
            emb = word.embedding.tolist() # Convert tensor to list
            all_neg_word_embeddings.setdefault(word_text, []).append(emb) # Append to the list of all negative word embeddings (has each occurrence of a negative emotion word)

            #Averaging embeddings for repeated negative emotion words in the text
            if word_text in neg_word_avg: #Store unique word averages for reference
                neg_word_avg[word_text] = np.mean(
                    [neg_word_avg[word_text], emb], axis = 0
                    )
            else:
                neg_word_avg[word_text] = emb

    #The embeddings for each occurrence of emotion words are stored in the dicts "all_pos_word_embeddings" and "all_neg_word_embeddings"
    #The lists of all occurrences of emotion words are stored in "all_occurrences_pos_words" and "all_occurrences_neg_words"
    #The average embeddings for each unique emotion word are stored in the dictionaries "pos_word_avg" and "neg_word_avg"
    #The unique emotion words are the keys of the dictionaries "pos_word_avg" and "neg_word_avg"

    #To compute cosine similarity between each occurrence of emotion words, 
    # we can use the dicts "all_pos_word_embeddings" and "all_neg_word_embeddings" 
    # which contain the embeddings for each occurrence of emotion words in the text.
    #This is to analyze the similarity between the emotion words used in the text. This can be done using a function like scipy.s

    #Save the repeated emotion words in the text in a list (has duplicates) 
    #and their embeddings in another list (has duplicates) to compute cosine similarity between each occurrence of emotion words
    def compute_cosine_similarity(word_embeddings):
        results = {}
        for word, emb_list in word_embeddings.items():
            if len(emb_list) >= 2:
                sims = cosine_similarity(np.array(emb_list)) #cosine_similarity needs a numpy array as input
                #Avoid self-pairs
                iu = np.triu_indices(len(emb_list), k =1)
                results[word] = {
                    'n_occurrences': len(emb_list), # The number of times (n) an emotion word appears in the text
                    'pairwise_sims': sims[iu].tolist(), #produce nxn matrix of cosine similarities between the occurrences and extract the upper triangle  - no diagonal, no duplicates
                    'mean_sim': sims[iu].mean() #average of the n pairwise cosine similarities : a mean close to 1.0 means the contexts were very similar
                }
        return results

    pos_similarity_results = compute_cosine_similarity(all_pos_word_embeddings)
    # print("Positive similarity results:", pos_similarity_results)
    neg_similarity_results = compute_cosine_similarity(all_neg_word_embeddings)
    # print("Negative similarity results:", neg_similarity_results)

    #Compute granularities using *all occurrences* of emotion words
    pos_granularity = compute_granularity(pos_word_avg)
    neg_granularity = compute_granularity(neg_word_avg)    

    #Extract unique emotion words found in the text #list already returns the unique emotion words
    ...
    return pd.Series({
            ...
            'positive_similarity_results_lemm': pos_similarity_results, #Cosine similarity results for positive emotion words
            ...
            'negative_similarity_results_lemm': neg_similarity_results #Cosine similarity results for negative emotion words
        })
...
text_level_initial_dataset.to_csv(
    r"Cosine_similarity_results.csv",
    index=False,
    encoding='utf-8-sig'
)
